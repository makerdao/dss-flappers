...
* `burn` - The percentage of the `vow.bump` to be moved to the underlying `flapper`. For example, a value of 0.70 \* `WAD` corresponds to funneling 70% of the `DAI` to the burn engine. Note that `FlapperUniV2` and `FlapperUniV2SwapOnly` do not support very small `burn` values.
